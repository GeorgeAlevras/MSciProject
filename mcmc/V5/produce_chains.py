import numpy as np
from generate_data import generate_data_seasyhrvd
...
def generated_data_seasyhrvd(model_params, state_params, added_noise_f=0.05):
    '''
        model_params = (b_a, b_sy, gamma_a, gamma_sy, gamma_h, exposed_rate_a, exposed_rate_sy, a, h, vaccination_rate, h_mortality_rate, std_f)
    ...
    generated_model_hospitalised = np.random.normal(generated_model_noise[4], added_noise_f*generated_model_noise[4])
    generated_model_recovered = np.random.normal(generated_model_noise[5], added_noise_f*generated_model_noise[5])
    generated_model_vaccinated = np.random.normal(generated_model_noise[6], added_noise_f*generated_model_noise[6])
    generated_model_deceased = np.random.normal(generated_model_noise[7], added_noise_f*generated_model_noise[7])
    data_std = np.std(generated_model_susceptible) + np.std(generated_model_exposed) + np.std(generated_model_asymptomatic) + \
         np.std(generated_model_symptomatic) + np.std(generated_model_hospitalised) + np.std(generated_model_recovered) + \
    ...
    return generated_model_no_noise, data_std, generated_model_susceptible, generated_model_exposed, generated_model_asymptomatic, \
        generated_model_symptomatic, generated_model_hospitalised, generated_model_recovered, generated_model_vaccinated, generated_model_deceased
    

def mcmc_seasyhrvd(model_params, state_params, gen_susc, gen_exp, gen_asym, gen_sym, gen_hosp, gen_rec, gen_vac, gen_dec, gen_std, iterations, std_f, chains=4, burn_in=1000, p=0.2):
    ...
    old_results = np.array([run_model('seasyhrvd', params, state_params)[0] for params in old_params])
    old_susceptible = old_results[:,0]
    old_exposed = old_results[:,1]
    old_asymptomatic = old_results[:,2]
    old_symptomatic = old_results[:,3]
    old_hospitalised = old_results[:,4]
    old_recovered = old_results[:,5]
    old_vaccinated = old_results[:,6]
    old_deceased = old_results[:,7]
    old_chi = np.array([chi_sq(gen_susc, i) + chi_sq(gen_exp, j) + chi_sq(gen_asym, k) + chi_sq(gen_sym, l) + chi_sq(gen_hosp, m) + chi_sq(gen_rec, n) + \
        chi_sq(gen_vac, o) + chi_sq(gen_dec, p) for i, j, k, l, m, n, o, p in zip(old_susceptible, old_exposed, old_asymptomatic, \
            old_symptomatic, old_hospitalised, old_recovered, old_vaccinated, old_deceased)])
    ...
    for i in range(iterations):
        ...
        new_results = np.array([run_model('seasyhrvd', params, state_params)[0] for params in new_params])
        new_susceptible = new_results[:,0]
        new_exposed = new_results[:,1]
        new_asymptomatic = new_results[:,2]
        new_symptomatic = new_results[:,3]
        new_hospitalised = new_results[:,4]
        new_recovered = new_results[:,5]
        new_vaccinated = new_results[:,6]
        new_deceased = new_results[:,7]
        new_chi = np.array([chi_sq(gen_susc, i) + chi_sq(gen_exp, j) + chi_sq(gen_asym, k) + chi_sq(gen_sym, l) + chi_sq(gen_hosp, m) + chi_sq(gen_rec, n) + \
        chi_sq(gen_vac, o) + chi_sq(gen_dec, p) for i, j, k, l, m, n, o, p in zip(new_susceptible, new_exposed, new_asymptomatic, \
            new_symptomatic, new_hospitalised, new_recovered, new_vaccinated, new_deceased)])
...
if __name__ == '__main__':
    ...
    generated_model_no_noise, gen_std, generated_model_susceptible, generated_model_exposed, generated_model_asymptomatic, \
        generated_model_symptomatic, generated_model_hospitalised, generated_model_recovered, generated_model_vaccinated, generated_model_deceased = generated_data_seasyhrvd(model_params, state_params)
    accepted, first = mcmc_seasyhrvd(model_params, state_params, generated_model_susceptible, generated_model_exposed, \
        generated_model_asymptomatic, generated_model_symptomatic, generated_model_hospitalised, generated_model_recovered, generated_model_vaccinated, \
            generated_model_deceased, gen_std, iterations=args.iterations, std_f=args.std, chains=args.chains, burn_in=args.burn_in, p=args.probability)
