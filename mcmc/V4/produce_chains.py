...
def generated_data_seasyrvd(model_params, state_params, added_noise_f=0.05):
    '''
        model_params = (b_a, b_sy, gamma_a, gamma_sy, exposed_rate_a, exposed_rate_sy, a, vaccination_rate, mortality_rate, std_f)
    ...
    generated_model_recovered = np.random.normal(generated_model_noise[4], added_noise_f*generated_model_noise[4])
...
def mcmc_seasyrvd(model_params, state_params, gen_susc, gen_exp, gen_asym, gen_sym, gen_rec, gen_vac, gen_dec, gen_std, iterations, std_f, chains=4, burn_in=1000, p=0.2):
    ...
    old_results = np.array([run_model('seasyrvd', params, state_params)[0] for params in old_params])
    old_susceptible = old_results[:,0]
    old_exposed = old_results[:,1]
    old_asymptomatic = old_results[:,2]
    old_symptomatic = old_results[:,3]
    old_recovered = old_results[:,4]
    old_vaccinated = old_results[:,5]
    old_deceased = old_results[:,6]
    old_chi = np.array([chi_sq(gen_susc, i) + chi_sq(gen_exp, j) + chi_sq(gen_asym, k) + chi_sq(gen_sym, l) + chi_sq(gen_rec, m) + \
        chi_sq(gen_vac, n) + chi_sq(gen_dec, o) for i, j, k, l, m, n, o in zip(old_susceptible, old_exposed, old_asymptomatic, \
            old_symptomatic, old_recovered, old_vaccinated, old_deceased)])
    ...
    for i in range(iterations):
        ...
        new_recovered = new_results[:,4]
        new_vaccinated = new_results[:,5]
        new_deceased = new_results[:,6]
...
if __name__ == '__main__':
    ...
    generated_model_no_noise, gen_std, generated_model_susceptible, generated_model_exposed, generated_model_asymptomatic, \
        generated_model_symptomatic, generated_model_recovered, generated_model_vaccinated, generated_model_deceased = generated_data_seasyrvd(model_params, state_params)
    accepted, first = mcmc_seasyrvd(model_params, state_params, generated_model_susceptible, generated_model_exposed, \
        generated_model_asymptomatic, generated_model_symptomatic, generated_model_recovered, generated_model_vaccinated, \
