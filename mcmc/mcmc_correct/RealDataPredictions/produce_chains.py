...
def mcmc(model_params, model_hyperparams, real_infected, temperature=1, iterations=20000, chains=6, \
    ...
    for i in range(iterations):
        ...
        new_chi += np.array([0.5*((new_params[c][4] - 0.16667)/0.016667)**2 for c in range(chains)])
        new_chi += np.array([0.5*((new_params[c][5] - 0.003428)/0.0003428)**2 for c in range(chains)])
        new_chi += np.array([0.5*((new_params[c][6] - 0.025)/0.0025)**2 for c in range(chains)])
        ...
        gelman_rubin.append([V_1/W_1, V_2/W_2, V_3/W_3, V_4/W_4, V_5/W_5, V_6/W_6, V_7/W_7])
