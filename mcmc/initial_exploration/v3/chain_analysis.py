...
if __name__ == '__main__':
    ...
    best_params = [float(best_stats[3]), float(best_stats[8]), float(best_stats[13]), float(best_stats[18]), float(best_stats[23])]
    ...
    plt.plot(t, generated_model_no_noise[2], label='Underlying Gen Data Signal')
    model_results_best = np.array(run_model('seirvd', best_params, new_states)[0])
    infected_best = model_results_best[2]
    ...
    g = plots.get_single_plotter(width_inch=6)
    g.plot_1d(samples, 'p4', marker=best_params[3])
    g.export('./Images/p4_dist.png')
    g = plots.get_single_plotter(width_inch=6)
    g.plot_1d(samples, 'p5', marker=best_params[4])
    g.export('./Images/p5_dist.png')
    print('\n')
    ...
    g = plots.getSinglePlotter()
    g.plot_2d(samples, ['p1', 'p4'], shaded=True)
    g.export('./Images/p1p4.png')
    g = plots.getSinglePlotter()
    g.plot_2d(samples, ['p1', 'p5'], shaded=True)
    g.export('./Images/p1p5.png')
    g = plots.getSinglePlotter()
    g.plot_2d(samples, ['p2', 'p3'], shaded=True)
    g.export('./Images/p2p3.png')
    g = plots.getSinglePlotter()
    g.plot_2d(samples, ['p2', 'p4'], shaded=True)
    g.export('./Images/p2p4.png')
    g = plots.getSinglePlotter()
    g.plot_2d(samples, ['p2', 'p5'], shaded=True)
    g.export('./Images/p2p5.png')
    g = plots.getSinglePlotter()
    g.plot_2d(samples, ['p3', 'p4'], shaded=True)
    g.export('./Images/p3p4.png')
    g = plots.getSinglePlotter()
    g.plot_2d(samples, ['p3', 'p5'], shaded=True)
    g.export('./Images/p3p5.png')
    g = plots.getSinglePlotter()
    g.plot_2d(samples, ['p4', 'p5'], shaded=True)
    g.export('./Images/p4p5.png')
    ...
    g.triangle_plot([samples], ['p1', 'p2','p3', 'p4', 'p5'], 
        filled_compare=True, 
    ...
    print(samples.PCA(['p1','p2','p3', 'p4', 'p5']))
    print('\n')
    ...
    for conf, lim0, lim1, lim2, lim3, lim4 in zip(samples.contours, lims0, lims1, lims2, lims3, lims4):
