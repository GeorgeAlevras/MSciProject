...
if __name__ == '__main__':
    ...
    generated_model_asymptomatic_vac = np.load('ExperimentData/generated_model_asymptomatic_vac.npy', allow_pickle=True)
    generated_model_symptomatic_vac = np.load('ExperimentData/generated_model_symptomatic_vac.npy', allow_pickle=True)
    generated_model_hospitalised = np.load('ExperimentData/generated_model_hospitalised.npy', allow_pickle=True)
    generated_model_hospitalised_vac = np.load('ExperimentData/generated_model_hospitalised_vac.npy', allow_pickle=True)
    ...
    best_params = [float(best_stats[3+5*i]) for i in range(0, 19)]
    
    plt.figure(1)
    X = ['b_a', 'b_sy', 'g_a', 'g_sy', 'g_h', 'e_a', 'e_sy', 'a', 'h', 'v', 'b_a_v', 'b_sy_v', 'g_a_v', 'g_sy_v', 'g_h_v',\
         'a_v', 'h_v', 'd_h', 'd_h_v']
    ...
    model_results_best = np.array(run_model('seasyhrva_vsy_vh_vd', best_params, new_states)[0])
    ...
    g.triangle_plot([samples], ['p1', 'p2','p3', 'p4', 'p5', 'p6', 'p7', 'p8', 'p9', 'p10', 'p11', 'p12', 'p13', 'p14', 'p15', \
        'p16', 'p17', 'p18', 'p19'], 
        filled_compare=True, 
    ...
    for i in range(len(X)):
        print(samples.getInlineLatex('p'+str(i+1),limit=2))
    print(samples.getTable().tableTex())
    print(samples.PCA(['p1', 'p2','p3', 'p4', 'p5', 'p6', 'p7', 'p8', 'p9', 'p10', 'p11', 'p12', 'p13', 'p14', 'p15', 'p16', \
        'p17', 'p18', 'p19']))
    print('\n')
    ...
    lims9 = stats.parWithName('p10').limits
    lims10 = stats.parWithName('p11').limits
    lims11 = stats.parWithName('p12').limits
    lims12 = stats.parWithName('p13').limits
    lims13 = stats.parWithName('p14').limits
    lims14 = stats.parWithName('p15').limits
    lims15 = stats.parWithName('p16').limits
    lims16 = stats.parWithName('p17').limits
    lims17 = stats.parWithName('p18').limits
    lims18 = stats.parWithName('p19').limits
    for conf, lim0, lim1, lim2, lim3, lim4, lim5, lim6, lim7, lim8, lim9, lim10, lim11, lim12, lim13, lim14, lim15, lim16, \
        lim17, lim18 in zip(samples.contours, lims0, lims1, lims2, lims3, lims4, lims5, lims6, lims7, lims8, lims9, lims10, \
            lims11, lims12, lims13, lims14, lims15, lims16, lims17, lims18):
        ...
        print('p10 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim9.lower, lim9.upper, lim9.limitType()))
        print('p11 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim10.lower, lim10.upper, lim10.limitType()))
        print('p12 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim11.lower, lim11.upper, lim11.limitType()))
        print('p13 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim12.lower, lim12.upper, lim12.limitType()))
        print('p14 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim13.lower, lim13.upper, lim13.limitType()))
        print('p15 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim14.lower, lim14.upper, lim14.limitType()))
        print('p16 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim15.lower, lim15.upper, lim15.limitType()))
        print('p17 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim16.lower, lim16.upper, lim16.limitType()))
        print('p18 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim17.lower, lim17.upper, lim17.limitType()))
        print('p19 %s%% lower: %.3f upper: %.3f (%s)'%(conf, lim18.lower, lim18.upper, lim18.limitType()))
