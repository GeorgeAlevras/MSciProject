...
def generated_data_seirvd(model_params, state_params, added_noise_f=0.05):
    '''
        model_params = (b, gamma, exposed_rate, vaccination_rate, mortality_rate, std_f)
    ...
    generated_model_infected = np.random.normal(generated_model_noise[2], added_noise_f*generated_model_noise[2])
    ...
    generated_model_recovered = np.random.normal(generated_model_noise[3], added_noise_f*generated_model_noise[3])
    generated_model_vaccinated = np.random.normal(generated_model_noise[4], added_noise_f*generated_model_noise[4])
    generated_model_deceased = np.random.normal(generated_model_noise[5], added_noise_f*generated_model_noise[5])
    data_std = np.std(generated_model_infected) + np.std(generated_model_susceptible) + np.std(generated_model_exposed) + \
         np.std(generated_model_recovered) + np.std(generated_model_vaccinated) + np.std(generated_model_deceased)

    return generated_model_no_noise, generated_model_infected, data_std, generated_model_susceptible, generated_model_exposed, \
        generated_model_recovered, generated_model_vaccinated, generated_model_deceased
    

def mcmc_seirvd(model_params, state_params, gen_infected, gen_susc,gen_exp, gen_rec, gen_vac, gen_dec, gen_std, iterations, std_f, chains=4, burn_in=1000, p=0.2):
    ...
    old_results = np.array([run_model('seirvd', params, state_params)[0] for params in old_params])
    old_infected = old_results[:,2]
    old_susceptible = old_results[:,0]
    old_exposed = old_results[:,1]
    old_recovered = old_results[:,3]
    old_vaccinated = old_results[:,4]
    old_deceased = old_results[:,5]
    old_chi = np.array([chi_sq(gen_infected, i) + chi_sq(gen_susc, j) + chi_sq(gen_exp, k) + chi_sq(gen_rec, l) + \
        chi_sq(gen_vac, m) + chi_sq(gen_dec, n) for i, j, k, l, m, n in zip(old_infected, old_susceptible, old_exposed, old_recovered, old_vaccinated, old_deceased)])
    ...
    for i in range(iterations):
        ...
        new_results = np.array([run_model('seirvd', params, state_params)[0] for params in new_params])
        new_infected = new_results[:,2]
        new_susceptible = new_results[:,0]
        new_exposed = new_results[:,1]
        new_recovered = new_results[:,3]
        new_vaccinated = new_results[:,4]
        new_deceased = new_results[:,5]
        new_chi = np.array([chi_sq(gen_infected, i) + chi_sq(gen_susc, j) + chi_sq(gen_exp, k) + chi_sq(gen_rec, l) + \
            chi_sq(gen_vac, m) + chi_sq(gen_dec, n) for i, j, k, l, m, n in zip(new_infected, new_susceptible, new_exposed, new_recovered, new_vaccinated, new_deceased)])
...
if __name__ == '__main__':
    ...
    generated_model_no_noise, generated_model_infected, gen_std, generated_model_susceptible, generated_model_exposed, \
        generated_model_recovered, generated_model_vaccinated, generated_model_deceased = generated_data_seirvd(model_params, state_params)
    accepted, first = mcmc_seirvd(model_params, state_params, generated_model_infected, \
        generated_model_susceptible, generated_model_exposed, generated_model_recovered, generated_model_vaccinated, \
