...
for idx, file in enumerate(file_names):
    ...
    outChain[:,0] = 1.
    np.savetxt('./Converted_Files/convert_{}.txt'.format(idx+1),outChain)

samples = loadMCSamples('./Converted_Files/convert', settings={'ignore_rows':1000})

# 1D marginalized plot
g = plots.get_single_plotter(width_inch=6)
g.plot_1d(samples, 'p1')
g.export('./GetDistImages/p1_dist.png')
g = plots.get_single_plotter(width_inch=6)
g.plot_1d(samples, 'p2')
g.export('./GetDistImages/p2_dist.png')
g = plots.get_single_plotter(width_inch=6)
g.plot_1d(samples, 'p3')
g.export('./GetDistImages/p3_dist.png')
print('\n')

g = plots.getSinglePlotter()
g.plot_2d(samples, ['p1', 'p2'], shaded=True)
g.export('./GetDistImages/p1p2.png')
g = plots.getSinglePlotter()
g.plot_2d(samples, ['p1', 'p3'], shaded=True)
g.export('./GetDistImages/p1p3.png')
g = plots.getSinglePlotter()
g.plot_2d(samples, ['p2', 'p3'], shaded=True)
g.export('./GetDistImages/p2p3.png')
