import numpy as np
from generate_data import generate_data_seirv
...
def generated_data_seirv(model_params, state_params, added_noise_f=0.05):
    '''
        model_params = (b, gamma, exposed_rate, vaccination_rate, std_f)
    ...
    generated_model_no_noise, N_no_noise = generate_data_seirv(model_params_no_noise, state_params)
    generated_model_noise = generate_data_seirv(model_params, state_params)[0]
    ...
    data_std = np.std(generated_model_infected) + np.std(generated_model_susceptible) + np.std(generated_model_exposed) + \
         np.std(generated_model_recovered) + np.std(generated_model_vaccinated)

    return generated_model_no_noise, generated_model_infected, data_std, generated_model_susceptible, generated_model_exposed, generated_model_recovered, generated_model_vaccinated
    

def mcmc_seirv(model_params, state_params, gen_infected, gen_susc, gen_exp, gen_rec, gen_vac, gen_std, iterations, std_f, chains=4, burn_in=1000, p=0.2):
    ...
    old_results = np.array([run_model('seirv', params, state_params)[0] for params in old_params])
    ...
    old_chi = np.array([chi_sq(gen_infected, i) + chi_sq(gen_susc, j) + chi_sq(gen_exp, k) + chi_sq(gen_rec, l) + \
        chi_sq(gen_vac, m) for i, j, k, l, m in zip(old_infected, old_susceptible, old_exposed, old_recovered, old_vaccinated)])
    ...
    for i in range(iterations):
        ...
        new_results = np.array([run_model('seirv', params, state_params)[0] for params in new_params])
        ...
        new_chi = np.array([chi_sq(gen_infected, i) + chi_sq(gen_susc, j) + chi_sq(gen_exp, k) + chi_sq(gen_rec, l) + \
            chi_sq(gen_vac, m) for i, j, k, l, m in zip(new_infected, new_susceptible, new_exposed, new_recovered, new_vaccinated)])
...
if __name__ == '__main__':
    ...
    generated_model_no_noise, generated_model_infected, gen_std, generated_model_susceptible, generated_model_exposed, \
        generated_model_recovered, generated_model_vaccinated = generated_data_seirv(model_params, state_params)
    accepted, first = mcmc_seirv(model_params, state_params, generated_model_infected, \
