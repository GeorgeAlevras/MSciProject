...
"""
    This file contains all the noisy (stochastic) compartmental models.
"""


def sirv_odes(t, x, b, g, v_e, N, std_f=0.05):
    S = x[0]  # Susceptible
    I = x[1]  # Infected
    R = x[2]  # Recovered
    V = x[3]  # Vaccinated

    b = np.random.normal(b, std_f*b)
    g = np.random.normal(g, std_f*g)
    v_e = np.random.normal(v_e, std_f*v_e)
...
def sirv_c_odes(t, x, b_a, b_sy, v, e_a, e_sy, a, g_a, g_sy, g_h, h, d_h, b_a_v, b_sy_v, a_v, h_v, g_h_v, d_h_v, g_a_v, g_sy_v, std_f):    
    S = x[0]  # Susceptible
    E = x[1]  # Exposed
    A = x[2]  # Asymptomatic
    Sy = x[3]  # Symptomatic
