...
"""
    This file is used to plot noisy (stochastic) compartmental models and see dynamics / time-evolutions of different compartments.
"""
...
def use_args(args):
    ...
    solutions = run_noisy_model('sirv_c', population=1000e3, R_0=3.5, vac_frac=0, nat_imm_rate=0, vac_ef=0.8, vaccination_rate=0.006, days=args.days, std_f=args.noise)[0]
    ...
    plt.title('SIRV-Complex Noisy Model')
    plt.xlabel('Time [days]')
    plt.ylabel('Proportion of Population')
    plt.legend()
    plt.savefig('Plots/sirv_c_noisy_'+str(args.noise)+'.png')
    plt.show()
